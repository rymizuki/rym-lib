...
import { QueryResultList, QueryRunnerInterface } from './interfaces'
  ...
  describe('Concurrency and async behavior', () => {
    ...
    describe('Promise handling', () => {
      it('should properly handle Promise chaining for async operations', async () => {
        const result = await runner
          ...
          .then((manyResult) => {
            ...
            return runner.one({
              filter: { id: { eq: manyResult.items[0]?.id } },
            })
          })
          .then((oneResult) => {
      ...
      it('should handle Promise.reject() from QuerySpecification components', async () => {
        ...
        await expect(failingRunner.many()).rejects.toThrow(
          'Source operation failed',
        )
        await expect(failingRunner.one()).rejects.toThrow(
          'Source operation failed',
        )
        await expect(
          failingRunner.find({ filter: { id: { eq: 1 } } }),
